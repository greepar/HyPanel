...
type SubscriptionResponse = { subscriptionToken: string; subscriptionUrl?: string | null }
/** The Server builds the link from the configured panel address; older Servers only return the token. */
const linkFor = (result: SubscriptionResponse) =>
  result.subscriptionUrl ?? `${location.origin}/s/${encodeURIComponent(result.subscriptionToken)}`
...
export function OwnSubscription({ api, setError }: { api: ApiClient; setError: (value: string) => void }) {
  ...
  useEffect(() => {
    void api.request<SubscriptionResponse>('/api/user/v1/subscription-token')
      .then(result => { setToken(linkFor(result)); setState('ready') }, () => setState('missing'))
  }, [])
  const rotate = async () => {
    ...
    setRotating(true)
    try {
      const result = await api.request<SubscriptionResponse>('/api/user/v1/subscription-token/rotate', { method: 'POST' })
      setToken(linkFor(result))
      setState('ready')
      setError('已签发新订阅链接。')
    } catch (reason) {
  ...
  return <section className="card panel usage-card">
    ...
    {state === 'loading' ? <Loading /> : token ? <SubscriptionLinks url={token} setError={setError} /> : <Empty title="还没有可显示的订阅链接" description="旧版本签发的链接无法再次显示，点击“签发订阅链接”获取新链接。" />}
  </section>
}

/** The user's Clash/Mihomo subscription link with a QR code. */
export function SubscriptionLinks({ url, setError }: { url: string; setError: (value: string) => void }) {
  const [qr, setQr] = useState('')
  // The link is a bearer credential: masked and QR-blurred until the owner asks to see it.
  const [linkVisible, setLinkVisible] = useState(false)
  const [qrVisible, setQrVisible] = useState(false)
  useEffect(() => {
    void QRCode.toDataURL(url, { width: 260, margin: 2, errorCorrectionLevel: 'M', color: { dark: '#171717', light: '#ffffff' } })
      .then(setQr)
      .catch(reason => setError(messageFor(reason, '无法生成二维码')))
  }, [url])
  const copy = async () => {
    try { await copyText(url); setError('订阅链接已复制。') }
  ...
  return <div className="subscription-delivery">
    <div className="subscription-link">
      ...
      <div className="masked-link">
        <code>{linkVisible ? url : maskLink(url)}</code>
        <button type="button" className="password-toggle" aria-label={linkVisible ? '隐藏链接' : '显示链接'} title={linkVisible ? '隐藏链接' : '显示链接'}
          aria-pressed={linkVisible} onClick={() => setLinkVisible(!linkVisible)}>
          {linkVisible ? <EyeOff size={16} /> : <Eye size={16} />}
        </button>
      </div>
      <div className="row-actions">
        <button className="button button-primary" type="button" onClick={() => void copy()}>复制链接</button>
        <a className="button button-secondary" href={`clash://install-config?url=${encodeURIComponent(url)}`}>一键导入 Clash</a>
      </div>
      <small className="muted">适用于 Clash Verge、Mihomo Party、ClashX Meta、Stash 等客户端；已包含分流规则，客户端会显示剩余流量和到期时间。</small>
    </div>
    <aside className="qr-panel">
      <span>扫码导入</span>
      {qr ? <button type="button" className={qrVisible ? 'qr-reveal revealed' : 'qr-reveal'} aria-label={qrVisible ? '隐藏二维码' : '显示二维码'}
        onClick={() => setQrVisible(!qrVisible)}>
        <img src={qr} alt="订阅二维码" />
        {!qrVisible && <span className="qr-cover"><Eye size={18} />点击显示</span>}
      </button> : <Loading label="生成二维码…" />}
    </aside>
  </div>
}

/** Keeps the host and the token's first and last four characters: https://host/s/ZPvq••••••••IGKs */
const maskLink = (url: string) => url.replace(/\/s\/([^/?#]{4})[^/?#]*([^/?#]{4})$/, (_, head: string, tail: string) => `/s/${head}••••••••${tail}`)
