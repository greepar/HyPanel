...
export type GlobalSettings = { agentUpdateDefaultPolicy: 'Manual' | 'Auto'; backendUpdateDefaultPolicy: 'Manual' | 'Auto'; githubMirrorBaseUrl: string | null; agentReleaseVersion: string | null; backendReleases: Record<string, string>; server: ServerUpdate; dataDirectory: string; databaseSizeBytes: number; panelUrl: string | null; currentUrl: string }
export type CertificateKind = 'Upload' | 'Path' | 'Acme'
export type AcmeChallenge = 'http' | 'tls' | 'cloudflare'
export type Certificate = { id: string; name: string; kind: CertificateKind; createdAtUtc: string; notBeforeUtc: string | null; expiresAtUtc: string | null; fingerprint: string | null; subject: string | null; san: string[]; usedBy: number; certificatePath: string | null; privateKeyPath: string | null; acmeEmail: string | null; acmeChallenge: AcmeChallenge | null; hasAcmeDnsToken: boolean; sourceCheckedAtUtc: string | null; sourceError: string | null }
