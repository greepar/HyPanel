...
export const emptyService = (type: BackendType = 'hysteria2'): ServiceForm => type === 'hysteria2'
  ...
  : type === 'xray'
    ? { backendType: type, name: '', version: '26.3.27', listenHost: '0.0.0.0', port: '443', clientId: '', clientEmail: '', realityPrivateKey: '', realityPublicKey: '', shortId: '', serverName: '', destination: '', fingerprint: 'chrome' }
...
export function formFor(service: Service): ServiceForm {
  ...
  if (service.backendType === 'xray') return { ...emptyService('xray'), ...base, realityPrivateKey: String(c.realityPrivateKey ?? ''), realityPublicKey: String(c.realityPublicKey ?? ''), clientId: String(c.clientId ?? ''), clientEmail: String(c.clientEmail ?? ''), shortId: String(c.shortId ?? ''), serverName: String(c.serverName ?? ''), destination: String(c.destination ?? ''), fingerprint: String(c.fingerprint ?? 'chrome') } as ServiceForm
...
export function payloadFor(form: ServiceForm): ServicePayload {
  ...
  if (form.backendType === 'hysteria2') {
    ...
    if (!form.clientId.trim() || !form.realityPrivateKey || !form.realityPublicKey.trim() || !form.shortId.trim() || !form.serverName.trim() || !form.destination.trim()) throw new Error('请完整填写 Xray 客户端和 REALITY 配置。')
    config = { ...common, clientId: form.clientId, clientEmail: form.clientEmail, flow: 'xtls-rprx-vision', realityPrivateKey: form.realityPrivateKey, realityPublicKey: form.realityPublicKey, shortId: form.shortId, serverName: form.serverName, destination: form.destination, fingerprint: form.fingerprint }
  } else {
