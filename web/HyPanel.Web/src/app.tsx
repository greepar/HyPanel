...
import { AppShell, Loading, messageFor, Notice } from './ui'
...
export function App() {
  ...
  if (checkingSession) return <AppShell theme={theme} setTheme={setTheme}><Loading label="正在验证会话…" /></AppShell>
  ...
  return <AppShell theme={theme} setTheme={setTheme} route={route} navigate={navigate} identity={bootstrap ? '初始管理员' : user?.username} role={role ?? 'User'} logout={() => void logout()}>
    ...
    {role === 'User' ? <SubscriptionPage api={api} setError={setError} /> : <AdminApp route={route} api={api} setError={setError} />}
  </AppShell>
}
...
function Login({ theme, setTheme, api, onLogin, onBootstrap }: { theme: Theme; setTheme: (value: Theme) => void; api: ApiClient; onLogin: (token: string, user: User) => void; onBootstrap: (token: string) => void }) {
  ...
  return <AppShell theme={theme} setTheme={setTheme}><div className="login-layout"><section className="login-copy"><p className="eyebrow">HyPanel Control Plane</p><h1>把分散的代理服务，收进一个清晰的控制面。</h1><p>安全管理节点、服务实例、用户授权和订阅交付。</p></section><form className="card login-card" onSubmit={event => void submit(event)}><div className="segmented"><button type="button" className={mode === 'account' ? 'active' : ''} onClick={() => setMode('account')}>账户登录</button><button type="button" className={mode === 'bootstrap' ? 'active' : ''} onClick={() => setMode('bootstrap')}>初始令牌</button></div><div><p className="eyebrow">安全访问</p><h2>{mode === 'account' ? '欢迎回来' : '进入初始管理模式'}</h2><p className="muted">{mode === 'account' ? '使用 HyPanel 账户继续。' : '令牌仅保存在当前浏览器会话。'}</p></div>{mode === 'account' ? <><label>用户名<input required autoComplete="username" value={form.username} onInput={event => setForm({ ...form, username: event.currentTarget.value })} /></label><label>密码<input required type="password" autoComplete="current-password" value={form.password} onInput={event => setForm({ ...form, password: event.currentTarget.value })} /></label></> : <label>Bearer 令牌<input required type="password" autoComplete="off" value={form.token} onInput={event => setForm({ ...form, token: event.currentTarget.value })} /></label>}{error && <Notice kind="error">{error}</Notice>}<button className="button button-primary button-wide" disabled={busy}>{busy ? '正在验证…' : '继续'}</button></form></div></AppShell>
}
